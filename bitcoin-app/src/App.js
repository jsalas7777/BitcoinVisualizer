...
import BitcoinPrice from "./BitcoinPriceComponent";
import BitcoinChain from "./BitcoinChain";
import BitcoinHashrate from "./BitcoinHashrate";
...
function App() {
  ...
  const contentView = () => {
    if (selection === "Price") {
      return <BitcoinPrice></BitcoinPrice>;
    }
    if (selection === "Chain") {
      return <BitcoinChain></BitcoinChain>;
    }
    if (selection === "Hashrate") {
      return <BitcoinHashrate></BitcoinHashrate>;
    }
  };
  ...
  return (
    <Box
      sx={{
        display: "flex",
        backgroundColor: "#000", // make the whole background black
      }}
    >
      {/* Sidebar */}
      <Drawer
